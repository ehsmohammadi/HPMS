...
namespace MITD.Presentation
{
    public static class WebClientHelper
    {
        ...
        private static Exception HandleException(WebException ex)
        {
            var s = readString(ex.Response.GetResponseStream());
            var exp2 = new Exception(s);
            exp2.Data.Add("error", ex.Response.Headers["exception"]);
            return exp2;
        }
        ...
        private static Exception convertException(WebException exp)
        {
            ...
            if (exp.Response is HttpWebResponse)
            {
                ...
                if (httpExp.StatusCode == HttpStatusCode.Unauthorized)
                {
                    ...
                    exp2.Data.Add("error", "Unauthorized Access");
                }
                else
                {
                    exp2.Data.Add("error", exp.Response.Headers["exception"]);
                }
            }
            return exp2;
        }
