...
{
    [KnownType(typeof(JobIndexInPeriodDTO))]
    [KnownType(typeof(JobIndexGroupInPeriodDTO))]
    [KnownType(typeof(AbstractIndexInPeriodDTOWithActions))]
    [JsonConverter(typeof(AbstarctJobIndexDtoConverter))]

    public partial class AbstractIndexInPeriodDTO
    {
    ...
    public class AbstarctJobIndexDtoConverter : JsonCreationConverter<AbstractIndexInPeriodDTO>
    {

        protected override AbstractIndexInPeriodDTO Create(Type objectType,
          JObject jObject)
        {
            var dtoType = jObject.Value<string>("DTOTypeName");
            if (dtoType.Equals("JobIndexInPeriodDTO"))
                return new JobIndexInPeriodDTO();
            else
                return new JobIndexGroupInPeriodDTO();
