...
", RuleType.PerCalculation, 4);
            ...
            using (var uow = uows.CurrentUnitOfWork as NHUnitOfWork)
            {
                ...
                var unitIndexCategory = new PMSAdmin.Domain.Model.UnitIndices.UnitIndexCategory(unitIndexRep.GetNextId(), null, "دسته شاخص فنی", "TechnicalUnitIndexCategory");
                unitIndexRep.Add(unitIndexCategory);

                var unitIndex1 = new PMSAdmin.Domain.Model.UnitIndices.UnitIndex(unitIndexRep.GetNextId(), unitIndexCategory,
                        "فرهنگ و تعهد سازمانی", "OrganizationalCultureAndCommitment");
                //unitIndex1.AssignCustomFields(jobIndexCftList);
                unitIndexRep.Add(unitIndex1);
